...
flowchart TB
    ...
    GW -- gRPC --> PUR
    GW -- gRPC --> INV
    PUR -. evento CompraLançada .-> MQ
    MQ -. consome evento .-> INV
```
...
- A seta `purchasing-service → RabbitMQ → inventory-service` representa o estado **alvo** do sistema (Fase 6 do TODO). No MVP inicial (Fase 4), essa comunicação começa como uma chamada gRPC síncrona direta entre os dois serviços, e só migra pra evento assíncrono via RabbitMQ mais adiante — o diagrama já mostra pra onde a arquitetura caminha, não o primeiro incremento. *(Atualizado em 2026-08-27: era NATS, trocado por RabbitMQ — ver ADR-0009.)*
